...
classdef SimScenario 
    ...
    properties
        ...
        egoSemActionLog             % 自车语意级行为记录
        egoSemAction = '-'          % 自车当前的语意级行为
        egoDoneAction               % 自车完成的语义行为

        % 场景代价权重
        w_safe_ttc1 = 2e3;         % 安全-->1/TTC 2500
        w_safe_thw = 25;          % 安全-->前后车间距2500
        ...
        % 额外的惩罚
        LC_penalty = 30;           % 每次换道会额外产生的消耗  60→2
        % 场景限制阈值
        ...
        cost_safe = 0;             % 安全性累计代价
        cost_effi = 0;             % 效率累计代价
        cost_smooth = 0;           % 平顺性累计代价
        cost_lanechange = 0;       % 换道累计代价
        cost_rule = 0;             % 规则累计代价
        cost_consist = 0;          % 决策一致性代价
    ...
    methods
        ...
        function obj = setAction(obj,actionName)
            ...
            switch actionName
                case 'L'
                    ...
                    obj.cost_lanechange = obj.cost_lanechange + obj.LC_penalty; % 变到内部啦→
                    obj.egoSemAction = 'L';
                    obj.egoDoneAction = [obj.egoDoneAction 'L'];

                case 'R'
                    inc_egoTargetLaneIdx = -1;
                    obj.egoLC_counter = obj.egoLC_counter + 1;
                    obj.vehState(1,obj.var_changingLane) = -obj.egoLC_counter;
                    obj.cost_lanechange = obj.cost_lanechange + obj.LC_penalty;
                    obj.egoSemAction = 'R';
                    obj.egoDoneAction = [obj.egoDoneAction 'R'];
                case 'K'
        ...
        %#########################################剪枝算法#################################################
        %#################################################################################################
        function flag = trim(obj,iffinal) % 剪枝算法，看是否需要把这个场景减掉
            if nargin < 2
                iffinal = false;
            end
            flag = false;
            ...
            if obj.ifSceParallel % 如果是场景并行计算
                if obj.cost > obj.cost_compare
                    flag = true;
                    disp(['--------场景' obj.allowedPattern obj.egoSemActionLog '由于代价超过对比值被剪枝，' ...
                        '当前已经完成' char(48+obj.egoLC_counter) '次换道'])
                elseif iffinal % 如果终局
                    if ~strcmpi(obj.egoDoneAction,obj.allowedPattern) % 未完成
                        flag = true;
                        disp(['--------场景' obj.allowedPattern obj.egoSemActionLog '由于未完成规定动作被剪枝，' ...
                            '当前已经完成' char(48+obj.egoLC_counter) '次换道'])
                    end
            % 如果距离路口到达给定距离，如果还没到规定车道，就要剪掉啦
                elseif obj.remDist_(1) < 30 && ~obj.canReachNextEdge_(1)
                    flag = true;
                    disp(['--------场景' obj.allowedPattern obj.egoSemActionLog '由于在路口30米内还没到规定车道被剪枝，' ...
                        '当前已经完成' char(48+obj.egoLC_counter) '次换道'])
                else%if length(obj.allowedPattern) ~= 1 % 如果只进行一次换道，就不考虑对不换道这种
                    ...
                    if (obj.maxSimTime - obj.getSimTime) < remainActionTime
                        ...
                        disp(['--------场景' obj.allowedPattern obj.egoSemActionLog '由于在剩余时间不够完成动作被剪枝，' ...
                            '当前已经完成' char(48+obj.egoLC_counter) '次换道'])
                        % end
                    end
                end
        ...
        function spdDesList = getAllSpdDes(obj) % 需要提前更新好remDist
            ...
            for vNo = 1:obj.vehNum
                ...
                % spdDesList(vNo) = min([spdDesList(vNo) obj.vehicles{vNo}.v_des]);
            ...
            for vNo = 2:obj.vehNum
                spdDesList(vNo) = obj.vehicles{vNo}.v_des;
            end
        ...
        function obj = simulate(obj) 
            ...
            obj.egoSemActionLog = [obj.egoSemActionLog obj.egoSemAction];
            obj.egoSemAction = '-';
            obj.ego_canLC = obj.getEgoCanLC;
        end
        ...
        function obj = calcCost(obj) % 计算新的一小节这期间的代价并累加到原代价上
            ...
            % 高效性---和期望速度的绝对差值
            SPD_DIFF_penalty = sum(max(mean(veh_spds) - veh_spds,0).^2);
            ...
            obj.cost_safe = obj.cost_safe + obj.w_gama_prod * j_safe;
            obj.cost_effi = obj.cost_effi + obj.w_gama_deriv * j_effi;
            obj.cost_rule = obj.cost_rule + obj.w_gama_deriv * j_rule;
            obj.cost_consist = obj.cost_consist + obj.w_gama_prod * j_consist;
            obj.cost_smooth = obj.cost_smooth + j_smooth;
            ...
            function weightedCost = calcWeightedCost(cost,weight,vehNumber)
                if vehNumber > 1
                    weightedCost = cost(1) + weight*max(cost(2:end));
                else
                    weightedCost = cost;
                end
            end
        ...
        function cost = get.cost(obj)
            cost = obj.cost_safe + obj.cost_effi + obj.cost_smooth ...
                + obj.cost_rule + obj.cost_consist + obj.cost_lanechange;
        end
        ...
        function showCost(obj)
            ...
            % 输出各代价项
            for i = 1:length(costNames)-1
                fprintf('%-20s | %12.2f\n', costNames{i}, costValues(i));
            end
            
            % 输出总代价
            fprintf('-----------------------------------------\n');
            fprintf('%-20s | %12.2f\n', costNames{end}, costValues(end));
            fprintf('-----------------------------------------\n');
            
            % 添加必要说明
            fprintf('注：\n');
            fprintf('1. 各代价项已按时间进行折扣/增益处理\n');
            fprintf('2. 安全性和一致性代价随时间衰减（γ=%.2f）\n', obj.w_gama);
            fprintf('3. 效率和规则代价随时间递增\n');
        end
