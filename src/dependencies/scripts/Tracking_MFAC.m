...
%     'Ny', 2, 'Nu', 2, 'Ly', 3, 'Lu', 3, ...
%     'rho', 0.7,...
%     'lambda', 0.1, 'mu',1, 'eta',0.1, ...
%     'u_cons', [ -3 2;-30/180*pi 30/180*pi], ...
%     'du_cons', [ -10 10;-180/180*pi 180/180*pi]*Ts, ...
%     'uplim', 3, 'phi_Lyp1', [-1 0;0 -1]*5};
% global mfac %#ok
% mfac = MFAC_FFDL_Controller(ctrlParams{:});
...
idx_tgt=0;
iter=0;
while idx_tgt<size(path,1) && iter < 170%当目标点的个数大于等于最终点时结束仿真循环
    iter=iter+1;
    %记录车辆状态
    position(iter,:)=pos;
    speed(iter)=spd;
    ...
    %获取前轮转角：控制量 %获取加速度
    [delta,ax]=getDelta(idx_tgt,path,head,pos,v_tgt,spd);
...
%% 子函数：获取下一时刻的车轮转角
function [delta,ax]=getDelta(idx_tgt,path,head,pos,v_tgt,spd)
global mfac %#ok
idx=idx_tgt;
if idx==size(path,1)
    idx=idx-1;
