% 模糊推理系统（Fuzzy Inference System ,FIS）建立
clc
clear
close
%% 构造输入、输出
%创建新的模糊推理系统
fis = mamfis; %生成Mamdani模糊推理系统，而非sugfis(Sugeno系统)

%输入1：fai_d
fis = addInput(fis,[0 1],'Name','fai_d');
%添加隶属度函数membership function,MF
...
%% 规则库
%第1~5列分别是：fai_d, fai_v, fai_h 规格权重，AND/OR选项
rulelist=[...
    1 1 2 1 1;
    1 2 3 1 1;
    1 3 4 1 1;
...
figure(2);
subplot(3,1,1);plotmf(fis,'input',1); %模糊系统第一个输入隶属函数图
subplot(3,1,2);plotmf(fis,'input',2); %模糊系统第二个输入隶属函数图
subplot(3,1,3);plotmf(fis,'output',1);%模糊系统输出隶属函数图
figure(3);gensurf(fis);          %模糊规则曲面

%% 保存
writeFIS(fis,'LCD'); %lane change decision
save FIS.mat fis
