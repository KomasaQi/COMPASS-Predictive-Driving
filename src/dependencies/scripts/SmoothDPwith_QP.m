...
for i=1:n_DP-1
 ...
 for i=1:n_DP
     for j=1:n_QP
         ...
         [lb,ub]=ObstacleBdy(Obs,[time,Sdp],Slim);
         %构造Ax<=b
 ...
 %对比Bspline线条效果
%  [routebspline,u]=Bspline(route,[route(1,1),route(end,1)],dt_QP,2);
figure(1)
hold on
for i=1:length(Obs)
...
plot(routeqp(:,1),routeqp(:,2),'g')
% plot(routebspline(:,1),routebspline(:,2),'b')
grid on
axis([0 5 0 80]);
xlabel('时间/s');
ylabel('路程/m');
