...
if __name__ == "__main__":
    ...
    if train:
        ...
        #     device = device,
        # )
        model = PPO.load(os.path.join(LOG_DIR, "best_model_128_full"), env=env, device=device)
        model.n_steps = 1024 // n_envs
        model.batch_size = 256
        model.learning_rate = cosine_annealing_schedule(1e-3, 1e-4)
        model.tensorboard_log = TENSORBOARD_LOG_DIR
        
        # Train the agent
        model.learn(total_timesteps=150 * 1000, progress_bar=True, callback=eval_callback)
        # Save the agent
        # 确保输出目录存在
        os.makedirs(LOG_DIR, exist_ok=True)
        model.save(os.path.join(LOG_DIR, "model_trans_128_002")) 
        env.close()
    
    # 测试环境也添加Monitor
    model = PPO.load(os.path.join(LOG_DIR, "model_trans_128_002"), device='cpu')
    # 创建测试环境（包含Monitor）
