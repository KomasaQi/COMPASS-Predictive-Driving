...
'BackgroundColor', 'none','FontSize', 12, 'FontWeight', 'bold', 'EdgeColor', 'none','HorizontalAlignment','center');
            ...
            obj.map_handle= imshow(obj.smallMap.getLocalMap([0 0],150), 'Parent', obj.mapAxes);
            hold on
        ...
        function refreshMap(obj,egoPos,egoHeading,radius)
            [img, xlimit, ylimit, xScale, yScale] = obj.smallMap.getLocalMap(egoPos(1:2),radius);
            % 获取图像大小
            ...
            % 计算显示区域在图像中的像素坐标
            xCenterPixel = (egoPos(1) - xlimit(1)) * xScale;
            yCenterPixel = (ylimit(2) - egoPos(2)) * yScale;
            radiusPixelX = radius * xScale; % 假设 x 和 y 方向的比例相同
            radiusPixelY = radius * yScale; % 假设 x 和 y 方向的比例相同
            ...
            mask = gaussianBlur(double(sqrt((X - carPos(1)).^2 + (Y - carPos(2)).^2) <= radiusPixelX),5);

            scale_factor = 150/obj.smallMap.patchMeterPerPixel;
            [x1,y1]=RotatePolygon(carPos(1)+scale_factor*[2/3,-1/3,-1/3],carPos(2)+scale_factor*[0,1/3,-1/3],carPos,-egoHeading);
