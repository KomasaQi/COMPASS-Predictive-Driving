for i = 1:10
    ...
    if ~mod(iterNum,10)
        ...
        if ~mod(iterNum,edge_junction_refreshStepInterval)
            ...
            [edgeList, junctionList] = getEntityInRange(proxyMat,ego.pos([1 2]),radius*10,ego.edgeID); % 找到新的相关实体
        ...
        trimmed_route = getTrimmedRoute(route,ego.pos(1:2),radius+250,ego.heading_cos_sin,scenario.Actors(1,1).Length*-0.85);
