...
function G_emb = embbedVehicles(G_local,ego,vehicleDummies,ref_ego)
    if nargin < 4 || isempty(ref_ego)
        ref_ego = ego;
    end
    ...
    nodesTable.nodes_pos = centralize2DPoints(nodesTable.nodes_pos,ref_ego.pos([1 2]),ref_ego.heading);
    riskMask = addRiskFeature(G_local.Edges.EndNodes,G_local.Edges.Weight,vehMask,vehSpdMask,ego.speed);
    nodesTable.riskMask = riskMask;
    G_emb = digraph(G_local.Edges,nodesTable);
end

function riskMask = addRiskFeature(endNodes,edgeWeight,occMask,spdMask,egoSpd)
    global params %#ok
    valid_link_no = edgeWeight <= 0;
    ds_links = endNodes(valid_link_no, :);
    src = ds_links(:,1);
    tgt = ds_links(:,2);

    nonVehIdxMask = occMask < 0.1;
    spdMask(nonVehIdxMask) = egoSpd;
    vehRelSpdMask = spdMask - egoSpd;
    vehRelSpdMask = sqrt(vehRelSpdMask).*sign(vehRelSpdMask);
    occfrontdecay = params.graph.risk.occfrontdecay;
    occbackdecay = params.graph.risk.occbackdecay;
    % 首先计算正向风险
    riskMaskOccFront = occMask;
    riskMaskOccBack = occMask;
    riskMaskMax = 1;
    for i = 1:params.graph.risk.occmaxhops
        % 静态碰撞风向沿道路方向扩散
        riskMaskOccFront(tgt) = min(occMask(tgt) + riskMaskOccFront(tgt)*(1-occfrontdecay) ...
            + riskMaskOccFront(src)*occfrontdecay,riskMaskMax);
        % 静态碰撞风向逆道路方向扩散
        riskMaskOccBack(src) = min(occMask(src) + riskMaskOccBack(src)*(1-occbackdecay) ...
            + riskMaskOccBack(tgt)*occbackdecay,riskMaskMax);
    end
    % 其次计算相对速度风险
    spddecay = params.graph.risk.spddecay;
    riskMaskSpdPos = zeros(size(spdMask));
    riskMaskSpdNeg = zeros(size(spdMask));
    riskMaskSpdPos(vehRelSpdMask > 0) = vehRelSpdMask(vehRelSpdMask > 0);
    riskMaskSpdNeg(vehRelSpdMask < 0) = vehRelSpdMask(vehRelSpdMask < 0);
    for i = 1:params.graph.risk.spdmaxhops
        riskMaskSpdPos(tgt) =  riskMaskSpdPos(tgt)*(1-spddecay) ...
            + riskMaskSpdPos(src)*spddecay; 
        riskMaskSpdNeg(src) =  riskMaskSpdNeg(src)*(1-spddecay) ...
            + riskMaskSpdNeg(tgt)*spddecay;
    riskMaskSpdPos(vehRelSpdMask > 0) = vehRelSpdMask(vehRelSpdMask > 0);
    riskMaskSpdNeg(vehRelSpdMask < 0) = vehRelSpdMask(vehRelSpdMask < 0);
    end

    % 将Occ风险和速度风险进行融合
    riskMask = min((riskMaskOccFront + riskMaskOccBack)/2 + params.graph.risk.spd2occ*(riskMaskSpdPos + riskMaskSpdNeg),riskMaskMax);
end
