        ...
        for resNum = 1:length(results)   
            ...
            if ~isempty(theResult)
                ...
                disp(['有效场景' num2str(validResultCounter) ': ' theResult.showAllowedPattern...
                    '，动作：' theResult.egoSemActionLog '，代价：' num2str(theResult.cost) '，调用序号：' num2str(resNum) ])
                theResult.showCost();
            end
        end
        disp(repmat('-- * ',1,15))
...
trimmed_route = getTrimmedRoute(route,ego.pos(1:2),radius+150,ego.heading_cos_sin,scenario.Actors(1,1).Length*-0.85);
        ...
        for simRound = 1:14 % 一共分为14个仿真阶段，推演之后20s的未来
            if ~isempty(scenarioSet)
                ...
                for sceNum = 1:last_validCounter
                    if ~last_scenarioSet{sceNum}.trim(simRound==14)
                ...
                disp(['场景' balancedSet{parSceNum} '第' num2str(simRound) '阶段，仿真到第' ...
                   num2str(currentTime) 's，此时有' num2str(validCounter) '种情况，代价分别为：'])
                for sceNum = 1:validCounter
                    disp(['————action:' scenarioSet{sceNum}.egoSemActionLog ' cost:' num2str(scenarioSet{sceNum}.cost)])
                end
