...
%     'ViewPitch',10,'ViewLocation',[scenario.Actors(1,1).Length*-3.5,0]); % Height*3, Length*-3.5
chasePlot(scenario.Actors(1,1),'Parent', gca,...
...
vehTrajDummies{1} = plot3(1:2,1:2,1:2,LineWidth=0.5,Color=[1 0 0]);
for idx = 2:maxVehNum+1
    vehTrajDummies{idx} = plot3(1:2,1:2,1:2,LineWidth=0.5,Color=[1 1 0]);
end
hold off
...
traci.vehicle.setLaneChangeMode(ego.vehID, 0b000000000000); % 限制不让车辆自主换道，但是可以sublanechange
