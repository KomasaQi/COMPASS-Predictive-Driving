...
class CompassHighwayEnv(gym.Env): 
    ...
    def __init__(self, config: dict = None, render_mode: str | None = None, verbose: bool = False) -> None:
        ...
        # Examine whether the SUMO_HOME environment variable is set
    ...
    def default_config(self) -> dict:
        config = {
            ...
            "reward":{ 
                ...
                "ttc_2d_inv_reward": -0.3, # the reward of 2D time-to-collision with all other vehicle # TODO: check the reward
                "hw_inv_reward": -0.02, # the reward of heading away of the front vehicle as well as left right front neihgbors # TODO: check the reward
            ...
            "simulation":{
                ...
                "save_state_rng": True, # whether to save the random state of the simulation
            },
            "gui":{
                "view": "View #0",
                "schema": "real world",
    ...
    def step(self, action):
        ...
        # Step the simulation
        traci.simulationStep(self.time + self.time_bias)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.config["simulation"]["save_state_rng"]:
            save_state_rng ="true"
        else:
        ...
        try:
            ...
            pass
        # 启动SUMO仿真
        print(os.path.join(self.config["files"]["test_case_root"], self.case_info["sumocfg_file"]))
        traci.start([self.render_cmd, '-c', 
                     os.path.join(self.config["files"]["test_case_root"], self.case_info["sumocfg_file"]),
                     '--start', '--quit-on-end','--seed',str(seed),"--save-state.rng", save_state_rng])  # 打开sumocfg文件  
        ...
        if self.render_mode == 'human':
            ...
            traci.gui.setSchema(ViewID, self.config["gui"]["schema"])
            traci.gui.setZoom(ViewID, self.config["gui"]["zoom"])
            traci.gui.trackVehicle(ViewID, ego_id)

        # 仿真到指定开始时间，确保自车出现
        self.time_bias = self.case_info["init_time"]
        traci.simulationStep(self.time_bias)
        revised_counter = 0
        while traci.vehicle.getRouteIndex(ego_id) < 0: # 确保自车出现（可能由于随机性，导致自车出现被阻塞）
            self.time_bias += 1
            revised_counter += 1
            traci.simulationStep(self.time_bias)
        ...
        # 订阅自车和周车相关变量
        traci.vehicle.subscribe(ego_id, self.veh_state_vars)
        ...
        self.ego_route = traci.vehicle.getRoute(ego_id)
    ...
    def _apply_action(self, action):
        ...
        if self.config['action']['type'] == 'continuous':
            ...
            traci.vehicle.setSpeed(egoID, speed_des_cmd)
            if lc_cmd is not None:
                traci.vehicle.changeLaneRelative(egoID, lc_cmd, settings['lc_time']) # 1: 相对车道变化，0: 绝对车道变化
    ...
    def _update_vehicle_state(self):
        ...
        """
        # 获取订阅变量
        veh_vars = traci.vehicle.getContextSubscriptionResults(self.config["egoID"])
        ego_vars = traci.vehicle.getSubscriptionResults(self.config["egoID"])  
    ...
    def _calc_ego_state(self):
        ...
        * **自车状态**:
        ...
        laneLength = traci.lane.getLength(ego_vars[tc.VAR_LANE_ID])
        ...
        if route_index == len(self.ego_route) - 1:
            ...
            next_lane_speedlim = traci.lane.getMaxSpeed(self.ego_route[route_index + 1] + "_0")
    ...
    def _reward(self, observation:Dict):
        ...
        * **奖励函数**: 
        ...
        lane_num = traci.edge.getLaneNumber(self.ego_vars[tc.VAR_ROAD_ID])
        ...
        safe_reward = ttc_inv_penalty + mei_reward + rttc_inv_reward + hw_penalty
    ...
    def render(self):
        ...
        # traci.gui.screenshot(viewID=self.config["gui"]["view"], filename="compass_env/output/screenshot.png", width=640,height=480)
        pass
    ...
    def _is_terminated(self) -> bool:
        ...
        return bool(speed_terminated or goal_terminated)
    ...
    def _can_reach_next_edge_multi(self, veh_id, extend_lane_num=1, current_lane=None, 
        ...
        lane_number = traci.edge.getLaneNumber(current_edge)
    ...
    def _calc_ttc(self, veh_id, clip=100.0, ego_speed=None, can_reach_multi=None, remLaneDist=None):
        ...
        if right_leader_cand:
            ...
            if self._valid_veh_id(min_right_leader_id):
                right_leader_speed = traci.vehicle.getSpeed(min_right_leader_id)
        ...
        if left_leader_cand:
            ...
            if self._valid_veh_id(min_left_leader_id):
                left_leader_speed = traci.vehicle.getSpeed(min_left_leader_id)
        ...
        if right_follower_cand:
            ...
            if self._valid_veh_id(min_right_follower_id):
                right_follower_speed = traci.vehicle.getSpeed(min_right_follower_id)
        ...
        if cand:
            ...
            if self._valid_veh_id(min_left_follower_id):
                left_follower_speed = traci.vehicle.getSpeed(min_left_follower_id)
    ...
    def lane_change_state_encoder(self, veh_id):
        ...
        # 分别获得左右换道状态并合并
        lc_state_L = traci.vehicle.getLaneChangeStatePretty(veh_id, tc.LANECHANGE_LEFT)[1]  # 取考虑TraCI指令的状态[1]
        lc_state_R = traci.vehicle.getLaneChangeStatePretty(veh_id, tc.LANECHANGE_RIGHT)[1]
        state_fdbk = tuple(dict.fromkeys(list(lc_state_L) + list(lc_state_R)).keys()) # 合并并去重
        
        # 检查是否可以向左右换道
        could_lc_L = traci.vehicle.couldChangeLane(veh_id, tc.LANECHANGE_LEFT)
        could_lc_R = traci.vehicle.couldChangeLane(veh_id, tc.LANECHANGE_RIGHT)
